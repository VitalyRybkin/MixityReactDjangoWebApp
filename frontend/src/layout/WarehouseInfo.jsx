...
export default function WarehouseInfo() {
    ...
    useEffect(() => {
        ;(async () => {
            await Promise.all([loadWarehouse(), loadContacts()])
        })()
    }, [id])
            ...
            <Stack spacing={2}>
                <Card variant="outlined" sx={{ width: '100%', borderRadius: 1 }}>
                    ...
                    <CardContent sx={{ p: 0 }}>
                        <Stack>
                            {[
                                { label: 'Наименование', value: warehouse?.name },
                                { label: 'Организация', value: warehouse?.organization },
                                { label: 'Адрес', value: warehouse?.address },
                                { label: 'Телефон', value: warehouse?.phoneNumber },
                                {
                                    label: 'Email',
                                    value: warehouse?.email ? (
                                        <EmailLink
                                            email={warehouse.email}
                                            sx={{ fontSize: '1.2rem', lineHeight: 1.1 }}
                                        />
                                    ) : null,
                                },
                                { label: 'Примечание', value: warehouse?.descriptions },
                            ].map((item) => (
